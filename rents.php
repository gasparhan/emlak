...
$sql = mysql_query("select * from `adverts` where `durum`='Kiralik' and `status`='Yayinda'")or die(mysql_error());
$num = mysql_num_rows($sql);
$flo = floor($num/14);
$rec = number_format(($num/14),0,".",",");
$flo > $rec ? $rec = $rec+1 : $rec = $rec; 
$sql = mysql_query("select * from `adverts` where `durum`='Kiralik' and `status`='Yayinda' order by `b_tarih` desc limit $list,14")or die(mysql_error());
echo "\n";
?>
		<div style="width:480px; margin-left:10px;">
			<div><img src="images/headers/rents.gif" width="150" height="20"></div>
				...
				<table cellpadding="0" cellspacing="0" border="0" width="480">
					...
					<tr>
						...
						<?
							...
							if ($page > 1){
								$class ="<a href=\"rents.php?page=".($page-1)."\" class=\"font_8d\">&laquo; önceki</a> - ";
							}else{
								$class = "<font class=\"font_cc\">&laquo; önceki</font> - ";
							}
							echo $class;
							if ($rec > $page){
								$class = "<a href=\"rents.php?page=".($page+1)."\" class=\"font_8d\">sonraki &raquo;</a>";
							}else{
